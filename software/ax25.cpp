...
static void update_crc(uint8_t a_bit) {
  // checksum for the end of the packet
  crc ^= a_bit;
  if (crc & 1) {
...
void ax25_send_flag() {
  // Flag is used to mark the end of a packet in AX.25 protocol: https://www.tapr.org/pdf/AX25.2.2.pdf#page=16
  uint8_t flag = 0x7e;
  int i; // TODO int can be smaller
...
void ax25_send_header(const struct s_address* addresses, int num_addresses) {
  // TODO num_addresses should be able to be removed and replaced with static 3
  int i, j;
  packet_size = 0;
  ones_in_a_row = 0;
  crc = 0xffff;
  ...
  // TODO figure out what these maths are doing
