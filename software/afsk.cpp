...
// PLAYBACK_RATE is imported from specific AFSK implementation and is MODEM_SPEED / 256
// TABLE_SIZE is imported from specific AFSK implementation and is the size of the sine table

// The actual baudrate after rounding errors will be:
// PLAYBACK_RATE / (integer_part_of((PLAYBACK_RATE * 256) / BAUD_RATE) / 256)
static const uint16_t BAUD_RATE = 1200; // defined by Bell 202 AFSK
...
volatile static uint16_t phase_delta;                    // 1200/2200 for standard AX.25 as defined by Bell 202 AFSK
volatile static uint16_t phase;                          // Fixed point 9.7 (2PI = TABLE_SIZE)
volatile static uint16_t packet_pos;                     // Next bit to be sent out
volatile static uint8_t sample_fifo[SAMPLE_FIFO_SIZE];   // first in first out queue of samples
...
inline static bool afsk_is_fifo_full() {
  // if (head + 1) % 32 (which would wrap back to 0 if head is at the last sample_fifo element in position 31) is equal to the tail, the sample_fifo is full
  return (((sample_fifo_head + 1) % SAMPLE_FIFO_SIZE) == sample_fifo_tail);
}

inline static bool afsk_is_fifo_full_safe() {
  // queries if fifo full after disabling interrupts to avoid race conditions
  noInterrupts();
...
inline static bool afsk_is_fifo_empty_safe() {
  // queries if fifo empty after disabling interrupts to avoid race conditions
  noInterrupts();
...
inline static void afsk_fifo_in(uint8_t s) {
  ...
  sample_fifo_head = (sample_fifo_head + 1) % SAMPLE_FIFO_SIZE; // mod 32 to act as wrap-around (ie 32 back to 0)
}

inline static void afsk_fifo_in_safe(uint8_t s) {
  // adds to sample_fifo after disabling interrupts to avoid race conditions
  noInterrupts();
  afsk_fifo_in(s);
  interrupts();
}
...
inline static uint8_t afsk_fifo_out() {
  ...
  sample_fifo_tail = (sample_fifo_tail + 1) % SAMPLE_FIFO_SIZE; // mod 32 to act as wrap-around (tail will never exceed sample_fifo size)
  return s;
}

inline static uint8_t afsk_fifo_out_safe() {
  // gets tail from sample_fifo and increments tail by 1 after disabling interrupts to avoid race conditions
  noInterrupts();
...
bool afsk_flush() {
  while (!afsk_is_fifo_full_safe()) {
    ...
    // Load up next bit ( if < 256)
    if (current_sample_in_baud < (1 << 8)) {
      if ((packet_pos & 7) == 0) {
        // if packet_pos is divisible by 8
        current_byte = afsk_packet[packet_pos >> 3];
        // load up next byte (packet_pos / 8)
      }
      else {
...
// Handled by the timer interrupts
AFSK_ISR{
