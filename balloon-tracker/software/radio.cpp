...
char mystatus[128] = "..::| Eric is so smart |::..";
...
{
  for(char i=0;i<3;i++) {
    Serial.println("sending connecet command...");
    ser.println("AT+DMOCONNECT");
  }
}
...
void setup()
{
  ...
  dorji_readback(dorji);
  Serial.println("DRA818V filter on");
